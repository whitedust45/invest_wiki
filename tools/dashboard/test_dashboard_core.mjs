...
const functions = [
  ...
  "isUsTicker",
  "marketDataSymbols",
  "marketDataQueryString",
  "normalizeCloseHistoryRows",
  "mergeCloseHistoryRows",
  "externalFlowOnDate",
  "buildDailySnapshot",
...
assert.equal(core.isUsTicker("QQQ"), true, "US tickers are recognized for local JSON sync");
assert.deepEqual(
  core.marketDataSymbols([
    { symbol: "000001" },
    { symbol: "QQQ" },
    { symbol: "000001" },
    { symbol: "" },
  ...
  ["000001", "QQQ"],
  "market data refresh uses deduplicated syncable symbols"
);
assert.equal(
  core.marketDataQueryString(["000001", "QQQ"], { days: 260, end_date: "2026-07-03" }),
  "symbols=000001%2CQQQ&days=260&end_date=2026-07-03",
  "market data API query includes symbols and history parameters"
);
assert.deepEqual(
  core.mergeCloseHistoryRows(
    [{ date: "2026-07-03", close: 10, source: "腾讯日线" }],
    [{ date: "2026-07-03", close: 11, source: "Tushare前复权日线" }]
  ),
  [{ date: "2026-07-03", close: 11, source: "Tushare前复权日线" }],
  "local/Tushare history overrides Tencent rows for the same date"
);
