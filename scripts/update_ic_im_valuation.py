...
DASHBOARD_JSON = Path("hybrid-barbell-dashboard/data/ic-im-valuation.json")
DEFAULT_HISTORY_DIR = Path("hybrid-barbell-dashboard/data/history")
ROLL_WATCH_DAYS = 10
ROLL_ALERT_DAYS = 5
...
def build_payload(history_dir: Path, years: int, include_basis: bool) -> dict:
    ...
    payload = {
        ...
        "source": {
            "current_valuation": "中证指数官网 /csindex-home/data-service/indexValuation",
            "current_pb_fallback": "东方财富 push2 quote fields f167/f152; best-effort only",
            "pe_history": "中证指数官网 /csindex-home/perf/indexCsiDsPe",
            "pb_history": "local CSV only; official public PB history endpoint not detected",
    ...
    for key, meta in INDEXES.items():
        ...
        pb = current.get("pb")
        pb_source = "csindex_current" if not is_missing(pb) else None
        fallback = {}
        if is_missing(pb):
        ...
        pe_values, pe_latest = pe_history(meta["code"], start, end)
        pe = pe_latest.get("peg") if pe_latest else current.get("pe")
        pe_source = "csindex_pe_history" if pe_latest else "csindex_current"
        ...
        pb_values = load_pb_history(history_dir / f"{meta['code']}_pb.csv")
        ...
        payload["indexes"][key] = {
            ...
            "trade_date": pe_latest.get("tradeDate") or current.get("tradeDate") or trade_date,
            "pe": pe,
            "pb": pb,
            "pe_source": pe_source,
            "pb_source": pb_source,
            ...
            "pb_percentile_source": "local_csv" if pb_values and pb_num is not None else None,
