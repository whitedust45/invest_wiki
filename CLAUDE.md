...
完整规范见 `.claude/skills/think/SKILL.md` Part A。
...
完整规范见 `.claude/skills/query/SKILL.md`。
...
| 命令 | 说明 |
|------|------|
| `bash .claude/skills/lint/scripts/lint-all.sh` | 一键健康检查 |
| `bash .claude/skills/query/scripts/search.sh topic <词>` | 按 topic 搜索 |
| `bash .claude/skills/query/scripts/search.sh fulltext <词>` | 全文搜索 |
| `bash .claude/skills/query/scripts/search.sh stale 90` | 找 90 天未更新页面 |
| `bash .claude/skills/ingest/scripts/save-raw.sh check <文件名>` | 验证 raw 文件名规范 |
