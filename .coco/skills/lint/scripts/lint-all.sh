...
# 用法：bash .coco/skills/lint/scripts/lint-all.sh
#
...
FOURTEEN_AGO=$(date -v-14d +%Y-%m-%d 2>/dev/null || date -d "14 days ago" +%Y-%m-%d 2>/dev/null) || { echo "ERROR: 无法计算 14 天前的日期，请检查 date 命令"; exit 1; }
...
NINETY_AGO=$(date -v-90d +%Y-%m-%d 2>/dev/null || date -d "90 days ago" +%Y-%m-%d 2>/dev/null) || { echo "ERROR: 无法计算 90 天前的日期，请检查 date 命令"; exit 1; }
for f in $(find "$WIKI_DIR/" -name "*.md" -not -name "index.md" -not -name "log.md"); do
