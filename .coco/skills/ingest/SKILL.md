...
1. **文件路径**：用户指定 `raw/` 下的已有文件
2. **粘贴文本**：用户直接贴入内容 → Agent 保存到 `raw/`
3. **URL**：用户给出链接 → Agent 必须先抓取内容、再写入 `raw/`,具体方式见下文「URL 抓取指引」

### URL 抓取指引（任何 Agent 通用）

收到 URL 后,按以下优先级选择抓取方式,目标是把可读正文落到 `raw/<规范文件名>.md`(或保留原扩展名)：
...
2. **回退到 shell 命令**
   ```bash
   # ---- HTML 网页 ----
   curl -sSL --max-time 30 -A "Mozilla/5.0" "<URL>" -o raw/.tmp.html
   # 优先 pandoc 转 Markdown
   pandoc -f html -t gfm --wrap=none raw/.tmp.html -o "raw/<生成的文件名>.md"
   # 没有 pandoc 时,退而用 readability-cli / html2text / w3m -dump 任一可用工具

   # ---- 纯文本 / Markdown / RSS 直链 ----
   curl -sSL --max-time 30 "<URL>" -o "raw/<生成的文件名>.md"

   # ---- PDF 链接 ----
   curl -sSL --max-time 30 "<URL>" -o "raw/<生成的文件名>.pdf"
   # 后续步骤通过 pdf skill 或 pdftotext 提取文本
   ```

3. **文件名生成**
   抓取前先调用脚本生成规范文件名（脚本会打印 `Target: raw/...`，不下载）：
   ```bash
   bash .coco/skills/ingest/scripts/save-raw.sh url "{URL}" "{title}"
   ```
   再把抓取结果写入 `Target` 行给出的路径，确保命名与「raw/ 文件命名规则」一致。
...
| 输入方式 | 命令 | raw/ 文件名格式 | 示例 |
|---------|------|----------------|------|
| 文件路径 | （直接复制到 `raw/`，保持原名） | 保持原文件名不变 | `raw/intelligent-investor-ch20.pdf` |
| 粘贴文本 | `... save-raw.sh paste "{title}" < content.txt` | `{YYYY-MM-DD}-{short-title}.md` | `raw/2026-05-13-margin-of-safety-notes.md` |
| URL | `... save-raw.sh url "{URL}" "{title}"`（先生成文件名再抓取写入） | `{YYYY-MM-DD}-{source-domain}-{short-title}.{ext}` | `raw/2026-05-13-goldman-china-internet.pdf` |
...
### 1. 阅读与理解
- **前置**：若输入为 URL,先按上文「URL 抓取指引」完成抓取并落盘到 `raw/`,再读取该文件
...
### 3. 创建来源摘要页
- 路径：`wiki/sources/{YYYY-MM-DD}-{short-title}.md`
...
| 情况 | 操作 |
|------|------|
| **新实体** | 在 `wiki/entities/` 创建新页面 |
| **新概念** | 在 `wiki/concepts/` 创建新页面 |
| **已有页面需更新** | 按增量更新规范修改（见 reference） |

### 5. Glossary 维护（新增步骤）

扫描素材中出现的术语，对照 `schema/glossary.md`：
...
### 7. 更新索引和日志
- 更新 `wiki/index.md`：新增/修改条目的 id + summary
- 追加 `wiki/log.md`：记录摄入了什么、影响了哪些页面
...
3. 从 `wiki/index.md` 删除对应条目
4. 在 `wiki/log.md` 追加 `[日期] update | 回退：{原因}`
5. 如果更新了 glossary → 恢复

## 辅助文件

- `scripts/save-raw.sh` — 保存原始素材到 raw/ 的脚本
