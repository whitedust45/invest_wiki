...
3. **文件名生成**
   ...
   再把抓取结果写入该路径,确保命名与「raw/ 文件命名规则」一致。
...
执行 `scripts/save-raw.sh` 完成文件保存（或手动按以下规则）：

| 输入方式 | raw/ 文件名格式 | 示例 |
|---------|----------------|------|
| 文件路径 | 保持原文件名不变 | `raw/intelligent-investor-ch20.pdf` |
| 粘贴文本 | `{YYYY-MM-DD}-{short-title}.md` | `raw/2026-05-13-margin-of-safety-notes.md` |
| URL | `{YYYY-MM-DD}-{source-domain}-{short-title}.{ext}` | `raw/2026-05-13-goldman-china-internet.pdf` |
...
- 追加 `wiki/log.md`：`## [YYYY-MM-DD] ingest | {素材标题}`，记录摄入了什么、影响了哪些页面
...
4. 在 `wiki/log.md` 追加 `## [YYYY-MM-DD] update | 回退：{原因}`
5. 如果更新了 glossary → 恢复
