...
### 对话沉淀来源（`type: dialogue`）处理

当候选页面的 `sources` 中包含 `type: dialogue` 时：

- 置信度天然为 **medium**（即使推理链完整，缺少外部权威背书）
- 输出中标注"来源：个人思考讨论（{date}）"
- 如果同一论点同时有 `type: dialogue` + 其他外部来源佐证，可提升为 **high**
- `depth: seed` 的内容置信度为 **low**，标注"种子想法，待深化"
