...
from modules.market_data.providers import (
    build_history_payload as build_market_history_payload,
    build_quotes_payload as build_market_quotes_payload,
)
from modules.market_data.store import (
    connect_market_data_db,
    export_position_history,
    export_position_quotes,
    finish_sync_run,
    record_source_event,
    start_sync_run,
    upsert_daily_bar,
...
DASHBOARD_JSON = ROOT / "apps" / "dashboard" / "data" / "position-quotes.json"
POSITION_HISTORY_JSON = ROOT / "apps" / "dashboard" / "data" / "position-history.json"
...
def main(argv: list[str]) -> int:
    ...
    if not args.symbols:
        print("missing symbols, e.g. modules/market_data/quotes.py 000568 000858", file=sys.stderr)
        return 2
    payload = build_market_quotes_payload(args.symbols)
    quote_errors = dict(payload.get("errors") or {})
    conn = connect_market_data_db()
    run_id = start_sync_run(conn, "quote", args.symbols, {"output": args.output})
    for quote in payload["quotes"].values():
        upsert_quote_snapshot(conn, quote, run_id)
    for symbol, error in quote_errors.items():
        record_source_event(conn, run_id, "error", "warn", str(error), symbol=symbol, source="dashboard-market-data", api_name="quote")
    finish_sync_run(
        conn,
        run_id,
        "success" if not quote_errors else "partial",
        {"quotes": len(payload["quotes"]), "errors": len(quote_errors)},
    )
    payload = export_position_quotes(conn, args.symbols)
    payload["errors"] = quote_errors
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    write_projection_record(conn, "position-quotes", output, run_id, len(payload["quotes"]), {"symbols": args.symbols})
    print(f"wrote {output} with {len(payload['quotes'])} quotes, {len(payload['errors'])} errors")
    if args.history_days > 0:
        history_payload = build_market_history_payload(args.symbols, args.history_days, args.end_date)
        history_errors = dict(history_payload.get("errors") or {})
        history_run_id = start_sync_run(
            conn,
            "history",
            args.symbols,
            {"history_days": args.history_days, "end_date": args.end_date, "history_output": args.history_output},
        )
        for symbol, rows in history_payload["histories"].items():
            for row in rows:
                upsert_daily_bar(conn, symbol, row, history_run_id, adjustment="qfq")
        for symbol, error in history_errors.items():
            record_source_event(conn, history_run_id, "error", "warn", str(error), symbol=symbol, source="dashboard-market-data", api_name="history")
        finish_sync_run(
            conn,
            history_run_id,
            "success" if not history_errors else "partial",
            {
                "rows": sum(len(rows) for rows in history_payload["histories"].values()),
                "errors": len(history_errors),
            },
        )
        history_payload = export_position_history(conn, args.symbols)
        history_payload["errors"] = history_errors
        ...
        write_projection_record(
            ...
            history_output,
            history_run_id,
            sum(len(rows) for rows in history_payload["histories"].values()),
            {"symbols": args.symbols, "history_days": args.history_days, "end_date": args.end_date},
        )
        print(
