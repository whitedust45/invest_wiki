...
/api/market-data/watchlist
/api/market-data/refresh-wiki
/api/market-data/jobs/<job_id>
/api/ledger
/api/ledger/backups
/api/sync/health
/api/sync/status
...
页面打开时会优先请求这个本地 API，实时运行 `modules/market_data/valuation.py` 更新 IC/IM 估值与贴水，并写回 `apps/dashboard/data/ic-im-valuation.json`。如果本地 API 不可用，页面会回退读取已有 JSON。

“刷新市场数据”按钮会提交后台任务到 `/api/market-data/refresh-wiki`，由 Python 本地服务异步刷新 wiki 中优秀公司/观察池 A 股标的的报价和历史行情，再通过 `/api/market-data/jobs/<job_id>` 轮询结果。长耗时抓取不阻塞浏览器交互，任务成功后页面重新读取 `position-quotes.json`、`position-history.json` 和 `ic-im-valuation.json`。

如果 `services/sync/.env` 配置了 `APP_ACCESS_KEY`，`/api/market-data/*` 接口也会要求同一个访问密钥；页面会优先使用浏览器里保存的云同步访问密钥，遇到 401 时提示输入。

`/api/market-data/watchlist` 会从以下 wiki 页面自动抽取 A 股标的：
...
A 股 / ETF 数据优先走 Tushare Pro；Token 统一写在 `services/sync/.env`：

```text
TUSHARE_TOKEN=你的 Tushare Token
```

实现不强制新增 Python 依赖：若本地安装了官方 `tushare` SDK，会优先通过 SDK 调用；若未安装 SDK，则直接 POST 官方 Tushare Pro HTTP API。若 Tushare 临时不可用，持仓价格和历史行情仍按原有免费源兜底。

市场数据长期事实库写入：

```text
apps/dashboard/data/market-data.db
```

`position-quotes.json`、`position-history.json`、`ic-im-valuation.json` 仍保留原 schema，作为从 SQLite 导出的前端投影层。SQLite 表关系设计见：

```text
docs/designs/market-data-sqlite-schema.md
```

`/api/ledger` 是统一本地服务保留的 SQLite 镜像备份接口：页面仍以浏览器 `localStorage` 为主存；只有通过 `python3 modules/ledger/local_service.py` 打开时，保存账本后才会异步追加一份快照到：
...
- 当前 PE / PB：优先 Tushare Pro `index_dailybasic`
- 当前 PE / PB 兜底：若 Tushare 或对应字段缺失，脚本会回退中证指数官网 `data-service/indexValuation`，再尝试东方财富 `push2` 行情字段 `f167/f152`
  - 这是 best-effort 兜底；如果东方财富接口断连或返回空值，脚本不中断
  - 东方财富只用于当前 PB/PE 展示兜底，不用于 PB 历史分位
- PE 历史分位：优先 Tushare Pro `index_dailybasic` 历史 PE；缺失时回退中证指数官网 `perf/indexCsiDsPe`
- PB 历史分位：优先 Tushare Pro `index_dailybasic` 历史 PB；缺失时回退本地 PB 历史 CSV
...
```bash
python3 modules/market_data/valuation.py --no-include-basis
```
...
```bash
python3 modules/market_data/quotes.py 000858 000568 600887 600153 600036 002818 002091 601668 600177 600873 601318 600938 600941 601225 000651 600690 512890 520890 159569 159545 513630 159117 QQQ QLD SPY
```
...
- A 股 / ETF 主源：Tushare Pro 官方 HTTP API
  - 股票日线：`daily` + `adj_factor` 生成前复权价格
  - ETF / 场内基金日线：`fund_daily`
- A 股 / ETF 兜底源：新浪 A 股行情 `hq.sinajs.cn`、腾讯行情 `qt.gtimg.cn`
- 美股：Yahoo chart 最新日收盘价，并用 USD/CNY 折算为人民币元
- 本地服务 API：`GET /api/position-quotes?symbols=000858,QQQ`
...
```bash
python3 modules/market_data/quotes.py 000568 000858 --history-days 260
```
...
- A 股 / ETF：通过“刷新市场数据”或 `GET /api/position-history?symbols=000568,000858&days=260` 更新 `position-history.json`；页面用腾讯日线 `appstock/app/fqkline/get` 补缺，再以本地历史数据覆盖同日记录，保证 Tushare / 本地 JSON 是优先口径
