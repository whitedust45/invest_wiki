...
const dashboardServiceCommand = "python3 modules/ledger/local_service.py";
const defaultPositionQuoteSymbols = ["000858", "000568", "600887", "600153", "600036", "002818", "002091", "601668", "600177", "600873", "601318", "600938", "600941", "601225", "000651", "600690", "512890", "520890", "159569", "159545", "513630", "159117", "QQQ", "QLD", "SPY"];
const positionQuotesCommand = `python3 modules/market_data/quotes.py ${defaultPositionQuoteSymbols.join(" ")}`;
const dashboardServiceBaseUrl = window.location.protocol === "file:"
  ? "http://127.0.0.1:8775/apps/dashboard/"
  : `${window.location.origin}/apps/dashboard/`;
...
function dashboardServiceUrl() {
  ...
  const currentOriginLooksUsable = ledgerServiceAvailable() && window.location.origin;
...
function localServiceGuidePanel() {
  ...
  const detail = fileMode()
    ? "浏览器直开本地 HTML 时，API 与部分 JSON 读取可能被限制。请在项目根目录启动本地服务后，从对应端口的 /apps/dashboard/ 入口访问页面。"
...
async function refreshLedgerBackups(options = {}) {
  ...
  try {
    ...
    ledgerBackupState = {
      status: "error",
      backups: [],
      message: "读取 SQLite 备份失败；请确认使用 python3 modules/ledger/local_service.py 启动。"
    };
    renderLedgerBackupPanel();
    if (!options.silent) {
    ...
    <section class="valuation-band panel">
      <div class="section-head">
        ...
        <div class="valuation-tools">
          ...
          <button id="refreshMarketData" type="button">刷新市场数据</button>
    ...
    <section class="panel position-panel">
      <div class="section-head">
        ...
        <div class="position-tools">
          ...
          <button type="button" data-action="refresh-market-data">刷新市场数据</button>
...
async function fetchAshareCloseHistory(symbols, startDate, endDate) {
  ...
  await Promise.all(symbols.map(async (symbol) => {
    ...
    try {
      result[symbol] = mergeCloseHistoryRows(await fetchTencentDailyHistory(symbol, endDate, days), localRows);
    } catch {
...
function marketDataSymbols(items = summarizeLedger().positions) {
  const symbols = new Set();
  (Array.isArray(items) ? items : []).forEach((item) => {
    const raw = typeof item === "string" ? item : item && item.symbol;
    const code = String(raw || "").trim();
    if (!code) return;
    if (marketCodeForAshare(code)) {
      symbols.add(code);
    } else if (isUsTicker(code)) {
      symbols.add(code.toUpperCase());
    }
  });
  return Array.from(symbols);
}

function marketDataQueryString(symbols, extra) {
  const params = new URLSearchParams();
  const cleanSymbols = marketDataSymbols(symbols);
  if (cleanSymbols.length) params.set("symbols", cleanSymbols.join(","));
...
async function loadLocalPositionQuotes(options = {}) {
  const symbols = marketDataSymbols(options.symbols || summarizeLedger().positions);
  const query = marketDataQueryString(symbols);
  const sources = options.preferApi === false
    ? [{ url: `./data/position-quotes.json?ts=${Date.now()}`, source: "json" }]
    : [
        { url: `/api/position-quotes?${query ? `${query}&` : ""}ts=${Date.now()}`, source: "api" },
        { url: `./data/position-quotes.json?ts=${Date.now()}`, source: "json" }
      ];
  let lastError = null;
  ...
  throw lastError || new Error("价格数据不可用");
}
...
async function loadPositionQuotesJson() {
  try {
    const applied = applyPositionQuotes(await loadLocalPositionQuotes());
    if (applied < 0) return;
    showToast(applied ? `已应用 ${applied} 条价格` : "价格 JSON 已读取，但没有匹配当前持仓代码", applied ? "success" : "info");
  } catch {
    showLocalServiceGuide("quotes");
    showToast("读取价格 JSON 失败：请先运行 modules/market_data/quotes.py", "error");
  }
}

async function loadPositionHistoryFromService(symbols, options = {}) {
  const days = Number.isFinite(Number(options.days)) ? Number(options.days) : 520;
  const query = marketDataQueryString(symbols, {
    days,
    end_date: options.endDate || options.end_date || ""
  });
  const response = await fetch(`/api/position-history?${query ? `${query}&` : ""}ts=${Date.now()}`);
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  return response.json();
}
...
async function fetchMarketDataApi(url, options = {}, timeoutMs = 2500) {
  const request = (key) => fetchWithTimeout(url, {
    ...options,
    headers: {
      ...(options.headers || {}),
      ...(key ? { "X-App-Key": key } : {})
  ...
  let response = await request(loadSyncAccessKey());
  if (response.status !== 401) return response;
  const key = promptSyncAccessKey();
  if (!key) return response;
  return request(key);
}

async function submitWikiMarketDataRefresh(options = {}) {
  const params = new URLSearchParams();
  params.set("days", String(Number.isFinite(Number(options.days)) ? Number(options.days) : 520));
  if (options.endDate || options.end_date) params.set("end_date", options.endDate || options.end_date);
  const response = await fetchMarketDataApi(`/api/market-data/refresh-wiki?${params.toString()}&ts=${Date.now()}`, {
    method: "POST",
    cache: "no-store"
  }, 2500);
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const payload = await response.json();
  if (!payload || !payload.job || !payload.job.id) throw new Error("后台任务未返回 job id");
  return payload.job;
}

async function fetchMarketDataJob(jobId) {
  const response = await fetchMarketDataApi(`/api/market-data/jobs/${encodeURIComponent(jobId)}?ts=${Date.now()}`, { cache: "no-store" }, 2500);
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const payload = await response.json();
  if (!payload || !payload.job) throw new Error("后台任务状态不可用");
  return payload.job;
}

async function waitForMarketDataJob(jobId, options = {}) {
  const timeoutMs = Number.isFinite(Number(options.timeoutMs)) ? Number(options.timeoutMs) : 180000;
  const intervalMs = Number.isFinite(Number(options.intervalMs)) ? Number(options.intervalMs) : 1500;
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
    const job = await fetchMarketDataJob(jobId);
    if (job.status === "success") return job;
    if (job.status === "failed") throw new Error(job.error || "后台刷新失败");
    await wait(intervalMs);
  }
  throw new Error("后台刷新超时");
}

async function refreshMarketData() {
  if (marketDataSyncing) {
    showToast("市场数据正在刷新", "info");
    return;
  }
  marketDataSyncing = true;
  showToast("已提交后台刷新任务", "info");
  let applied = 0;
  try {
    const job = await submitWikiMarketDataRefresh({ days: 520 });
    showToast(`后台刷新中：${job.id}`, "info");
    await waitForMarketDataJob(job.id);
    const quotePayload = await loadLocalPositionQuotes({ preferApi: false });
    applied = applyPositionQuotes(quotePayload);
    if (applied < 0) throw new Error("价格保存失败");
    localPositionHistoryCache = null;
    await loadLocalPositionHistoryPayload();
    await syncDailyHistory({ forceToday: true, render: false });
    await loadValuationJson({ silent: true, render: false });
    render();
    showToast(`市场数据后台刷新完成，应用 ${applied} 条价格`, "success");
  } catch {
    showLocalServiceGuide("quotes");
    showToast("市场数据后台刷新失败：请确认本地服务已启动", "error");
  } finally {
    marketDataSyncing = false;
  }
}
...
document.addEventListener("click", (event) => {
  ...
  if (action) {
    ...
    if (action.dataset.action === "refresh-market-data") {
      refreshMarketData();
      return;
    }
  ...
  if (event.target.closest("#refreshMarketData")) {
    refreshMarketData();
    return;
  }
...
async function loadValuationJson(options = {}) {
  ...
  for (const item of sources) {
    try {
      const response = await fetch(item.url);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      if (!applyValuation(await response.json(), { render: options.render !== false, source: item.source, silent: options.silent })) return false;
