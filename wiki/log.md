...
- 新增 `scripts/serve_dashboard.py`：
  - 在原静态页面基础上提供本地 API `/api/ic-im-valuation`
  - API 会实时运行 `scripts/update_ic_im_valuation.py`，更新并返回 IC/IM 估值与贴水 JSON
  - 默认服务地址仍为 `http://127.0.0.1:8775/hybrid-barbell-dashboard/index.html`
- 更新 `hybrid-barbell-dashboard/`：
...
- 更新 `scripts/update_position_quotes.py`：
...
- 更新 `scripts/update_position_quotes.py`：
  - 新增 `--history-days`，可生成 `hybrid-barbell-dashboard/data/position-history.json` 作为 A 股日收盘历史保底数据
...
- 更新 `hybrid-barbell-dashboard/`：
  ...
  - 新增 `scripts/update_position_quotes.py`，可生成 `hybrid-barbell-dashboard/data/position-quotes.json`，页面支持读取本地价格 JSON 更新估值层
...
- 更新 `scripts/update_ic_im_valuation.py`：
...
- 更新 `hybrid-barbell-dashboard/data/README.md`：补充东方财富兜底与移仓提醒口径
...
- 新增根目录策略网页：`hybrid-barbell-dashboard/`，用于手工填数后自动计算阶段、缺口和操作提示
- 新增 `scripts/update_ic_im_valuation.py`：从中证指数官网公开接口生成 IC/IM 估值 JSON；前端可读取当前 PE/PB 与 PE 分位，PB 分位仅在本地历史 CSV 可用时覆盖手工输入
