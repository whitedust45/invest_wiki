...
```markdown
✅ [脚本](../../tools/investing/ic_im_roll_discount_stress.py)
✅ 运行：python3 tools/investing/ic_im_roll_discount_stress.py
❌ 运行：python3 /Users/name/.../wiki实践/tools/investing/ic_im_roll_discount_stress.py
```

### wiki → raw（允许）

wiki 文档可以引用 raw 层的原文作为来源。使用相对路径（wiki 文件位于 `knowledge/wiki/` 子目录下，`knowledge/raw/` 是其兄弟目录）：

```markdown
原文详见：`../../raw/2026-05-15-investopedia-value-investing.md`
```
