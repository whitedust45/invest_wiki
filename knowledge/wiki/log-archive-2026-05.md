...
supersedes: []
superseded_by: []
see_also: [overview, log-archive-2026-06, log-archive-2026-05-early]
sources:
  - type: personal
...
- 新增可复用脚本：`tools/investing/ic_im_roll_discount_stress.py`
- 将“100 万起步、IC 底仓、极低估时单次加 IM、绝不爆仓优先”的讨论沉淀为长期执行框架
- 更新 `wiki/index.md`：收录组合页条目

## [2026-05-15] update | 压力测试脚本增强与相对路径规则固化

- 为 `tools/investing/ic_im_roll_discount_stress.py` 增加 PB 百分位触发判断与交互式输入模式
...
- 为 `tools/investing/ic_im_roll_discount_stress.py` 增加 `--decision-mode`，用于只输出“能否加 IM”与“需要额外补资多少”
...
- 为 `tools/investing/ic_im_roll_discount_stress.py` 增加 `--entry-signal-mode`，用于判断等待区 / 观察区 / 执行区
...
- 为 `tools/investing/ic_im_roll_discount_stress.py` 增加 `--auto-brief-mode`，按是否提供 `current-drop` 自动识别未建仓 / 已持有 IC 场景
...
- 让 `tools/investing/ic_im_roll_discount_stress.py` 默认从 [[personal-position-sizing-framework]] 读取观察区 / 执行区 / IM 候选阈值
